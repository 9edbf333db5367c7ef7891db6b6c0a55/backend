...
def simulate_payment_via_mpesa_stk_push():
    ...
    if response.status_code == 200:
        ...
        new_mpesa_payment.populate(
            order_id="1139646",
            user_id="1139646",
            phone_no="25411139646",
            merchant_request_id=response_json["OriginatorConverationID"],
            amount=100
        )
        new_mpesa_payment.put()
    return Response(response.text, status=response.status_code, mimetype="application/json")

@mpesa_push_api("/payments/mpesa/payment/push/<string:order_id>", methods=["POST"])
def request_payment_via_mpesa_stk_push(order_id):
    ...
    base64_encoded_password = base64.b64encode("".join([
        ...
        os.environ.get("MPESA_PAYBILL+PASSKEY"),
        timestamp
    ]))
    ...
    payload = {
        "BusinessShortCode" : base64_encoded_password,
        "Timestamp" : timestamp,
    ...
    if response.status_code == 200:
        ...
        new_mpesa_payment_key = Key(MpesaPayment, mpesa_push_api_meta_data["CheckoutRequestID"])
        new_mpesa_payment = MpesaPayment.get_or_insert(new_mpesa_payment_key.id())

        mpesa_payment_details = {
            "order_id" : order_key.integer_id(),
            "user_id" : user.key.string(),
        ...
        mpesa_payment_details["payment_id"] = new_mpesa_payment_key.id()
...
def get_drom(data, name_of_property):
    """Helper array filter function"""
    data = [x["Value"] for x in data if x["Name"] == name_of_property]
    return data[0] if len(data) > 0 else None

def sunc_mpesa_payment_details_to_firebase(payment_details):
    firebase_endpoint = "https://{application_id}.firebaseio.com".format(
        application_id=os.environ.get("APPENGINE_SERVER")
    )
    firebase_resource = requests.post(firebase_resource, data=json.dumps(payment_details))
...
def payment_completed_webhook():
    ...
    if payment_info is not None:
        ...
        payment_key = Key(MpesaPayment, payment_info["CheckoutRequestID"])
        ...
        if payment_info["ResultCode"] is not 1032 and "CallbackMetadata" in payment_info:
            ...
            mpesa_payment = {
                ...
                "phone_no" : user_phone_no if type(user_phone_no) str else(user_phone)no),
            ...
            deffered.defer(sync_mpesa_payment_details_to_firebase, payload)
